...
namespace Log		// работа с протоколом
{
	...
	void WriteLine(LOG log, const char* c, ...);			// вывести в протокол конкатекацию строк
	void WriteLine(LOG log, const wchar_t* c, ...);		// вывести в протокол конкатекацию строк
	...
	void WriteError(LOG log, Error::ERROR error);	// вывести в протокол информацию о ошибке
	void Close(LOG log);
};